package org.ephemq.cli;

import org.apache.commons.cli.*;
import org.ephemq.cli.cluster.ClusterListCommand;
import org.ephemq.cli.ledger.MigrateLedgerCommand;
import org.ephemq.cli.ledger.MigrateLedgerPlanCommand;
import org.ephemq.cli.support.Command;
import org.ephemq.cli.topic.TopicCreatedCommand;
import org.ephemq.cli.topic.TopicDeletedCommand;
import org.ephemq.cli.topic.TopicListCommand;
...
/**
 * The MeteorAdmin class serves as the main entry point for the ephemq CLI application.
 * It supports various administrative commands related to ephemq, such as listing topics,
...
public class EphemqAdmin {
     ...
     * and '-b' use compatible port (@see {@code org.ephemq.config.CommonConfig#getCompatiblePort})
    ...
    private static void main0(String[] args) {
        try {
                ...
                case 2 -> {
                    if (args[0].equals("help")) {
                        ...
                        if (cmd != null) {
                            ...
                            if (options != null) {
                                printCmdHelp(String.format("ephemq-cli admin %s", cmd.name()), options);
                                return;
                            }
                            System.out.printf("%s %s INFO - %s - The command[%s] does not exists", newDate(), Thread.currentThread().getName(), EphemqAdmin.class.getName(), args[1]);                        }
                    }
                }
                default -> {
                    ...
                    if (cmd != null) {
                        ...
                        if (cmdLine == null) {
                            System.out.printf("%s %s INFO - %s - The command[%s] does not exists", newDate(), Thread.currentThread().getName(), EphemqAdmin.class.getName(), args[0]);
                            return;
                        }
                        ...
                        if (cmdLine.hasOption('b')) {
                            ...
                            Client client = new Client("ephemq-cli-client", config, new CombineListener() {
                            });
                            try {
                                client.start();
            ...
            System.out.printf("%s %s ERROR - %s - %s", newDate(), Thread.currentThread().getName(), EphemqAdmin.class.getName(), t.getMessage());            System.exit(-1);
        }
        System.exit(0);
    }
