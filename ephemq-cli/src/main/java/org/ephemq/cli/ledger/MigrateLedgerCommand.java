...
import org.ephemq.common.util.StringUtil;
import org.ephemq.remote.proto.server.MigrateLedgerResponse;
...
public class MigrateLedgerCommand implements Command {
            ...
            if (commandLine.hasOption('e')) {
                ...
                if (!StringUtil.isNullOrEmpty(file)) {
                    ...
                    if (infos == null || infos.isEmpty()) {
                        System.out.printf("%s %s INFO %s - Migrate ledger successfully, partition ledger doex not exists%n", currentTime(), Thread.currentThread().getName(), MigrateLedgerCommand.class.getName());
                        return;
                    }
                    ...
                    for (MigrateLedger info : infos) {
                        try {
                            ...
                            throw new CommandException(String.format("Migrate ledger failure, and try again later - [topic: %s], [partition: %d]", info.getTopic(), info.getPartition()));
                        } catch (Exception e) {
                            System.out.printf("%s %s ERROR %s - %s", currentTime(), Thread.currentThread().getName(), MigrateLedgerCommand.class.getName(), e.getMessage());
                            retry(client, info.getTopic(), info.getPartition(), info.getFrom(), info.getTo());
                        }
                    }
                }
            } else {
                throw new IllegalArgumentException("ephemq-cli illegal argument exception, [-e] args cannot be empty.");
            }
        } catch (Throwable t) {
            System.out.printf("%s %s ERROR %s - %s", currentTime(), Thread.currentThread().getName(), MigrateLedgerCommand.class.getName(), t.getMessage());
    ...
    private void retry(Client client, String topic, int partition, String original, String destination) throws ExecutionException, InterruptedException {
        System.out.printf("Migrate ledger[topic: %s, partition: %d, original-broker: %s, destination-broker: %s] failed, and try again after 30 s, if you want to stop it, ant you can execute `Ctrl-C`", topic, partition, original, destination);
        ...
        Future<?> future = retry.submit(() -> {
            try {
                ...
                System.out.printf("Migrate ledger retry failure, and try again later - [topic: %s], [partition: %d]",  topic, partition);
