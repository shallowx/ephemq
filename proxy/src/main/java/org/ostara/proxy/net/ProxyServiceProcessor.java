...
import org.ostara.client.internal.MessageLedger;
...
import org.ostara.proxy.management.LedgerSyncManager;
import org.ostara.proxy.management.ProxyClusterManager;
import org.ostara.proxy.management.ProxyTopicManager;
import org.ostara.proxy.management.ProxyZookeeperManager;
...
public class ProxyServiceProcessor extends ServiceProcessor {
    ...
    protected void processQueryTopicInfos(Channel channel, int command, ByteBuf data, InvokeAnswer<ByteBuf> answer) {
        ...
        try {
            ...
            commandExecutor.execute(() -> {
                try {
                    List<Node> clusterUpNodes = proxyClusterManager.getClusterUpNodes();
    ...
    private NavigableMap<String, Integer> calculateReplicas(Channel channel, String topic, int ledger) {
        ...
        for (Node node : proxyClusterManager.getClusterUpNodes()) {
            if (node == null) {
                continue;
            }
    ...
    private NavigableMap<String, Integer> selectLedgerNodes(int replicaCount, String token, NavigableMap<String, Integer> nodes) {
        ...
        List<String> routeNodes = proxyClusterManager.route2Nodes(token, replicaCount);
        ...
        try {
            ...
            commandExecutor.execute(() -> {
                try {
                    ...
                    logManager.cleanSubscribe(channel, request.getLedger(), promise);
                } catch (Exception e) {
