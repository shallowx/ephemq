...
import org.ephemq.proxy.core.ProxyMetricsListener;
import org.ephemq.proxy.core.ProxyServerConfig;
import org.ephemq.proxy.remoting.MeteorProxyServer;
import org.ephemq.proxy.remoting.ProxySocketServer;
import org.ephemq.proxy.support.ProxyDefaultManager;
 ...
 * ephemq proxy server. It parses command line arguments, loads properties
 * from a specified configuration file, and initializes the server with the
 * appropriate settings.
 */
public class EphemqProxy {
    private static final InternalLogger logger = InternalLoggerFactory.getLogger(EphemqProxy.class);
    ...
    private static void start(MeteorServer server) {
        try {
            ...
            if (logger.isErrorEnabled()) {
                logger.error("Start ephemq proxy server failed", e);
            }
            System.exit(-1);
        }
    }
    ...
    /**
     * Constructs and initializes the command line options required for the ephemq proxy server.
     *
     * @return Options object containing the necessary command line options configured.
     */
    private static Options constructCommandlineOptions() {
        Options options = new Options();
        Option option = new Option("c", "configFile", true, "ephemq proxy server config file");
